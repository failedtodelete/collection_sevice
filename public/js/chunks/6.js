  ...
  methods: {
    submitForm: function submitForm() {
      ...
      this.$validator.validateAll().then(function (result) {
        if (result) {
          var payload = {
            checkbox_remember_me: _this.checkbox_remember_me,
            userDetails: {
              email: _this.email,
              password: _this.password
            },
            notify: _this.$vs.notify,
            closeAnimation: _this.$vs.loading.close
          }; // Loading

          _this.$vs.loading();

          _this.$store.dispatch('auth/login', payload).then(function (res) {
                      ...
                      [
                        _c("div", { staticClass: "p-8 login-tabs-container" }, [
                          _c("div", { staticClass: "vx-card__title mb-4" }, [
                            _c("h4", { staticClass: "mb-4" }, [_vm._v("Вход")]),
                            _vm._v(" "),
                            _c("p", { staticClass: "mb-8" }, [
                              _vm._v(
                                "Добро пожаловать, вы можете войти в систему используя почту и пароль"
                              )
                            ])
                          ]),
                          _vm._v(" "),
